...
function SortTh({ label, sortKey, activeSortKey, sortDir, onSort, icon }: SortThProps) {
  ...
  return (
    <th className="px-6 py-4 text-[10px] font-black tracking-[0.1em] uppercase text-ink-mute whitespace-nowrap">
      <button
...
export const ApplicationsTable = memo(function ApplicationsTable({
  ...
  const processed = useMemo(() => {
    let list = searches.filter((s) => {
      ...
      // Date filtering
      let matchDate = true;
      if (dateFilter !== 'all' && s.timestamp) {
        const appDate = new Date(s.timestamp);
        const now = new Date();
        ...
        <div className="flex flex-wrap items-center gap-3 w-full lg:w-auto">
          ...
          <div className="flex items-center bg-card border border-border rounded-2xl p-1 shadow-sm">
            {(['all', 'today', '7d', '30d'] as DateFilterType[]).map((t) => (
              <button
                key={t}
                onClick={() => { setDateFilter(t); setPage(1); }}
                className={cn(
                ...
                <tr>
                  <td colSpan={onBulkAction ? 6 : 5} className="px-6 py-20 text-center">
                    <div className="flex flex-col items-center gap-3">
                      ...
                      <div>
                        <p className="font-bold text-ink text-base">No record found</p>
                      ...
                      <button
                        onClick={() => { setQuery(''); setStatusFilter('all'); setDateFilter('all'); }}
